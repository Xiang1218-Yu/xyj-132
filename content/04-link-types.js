(function () {
  ...
  _.getLinkType = function getLinkType(url) {
    try {
  ...
  _.normalizeSuffix = function normalizeSuffix(raw) {
    if (!raw) return '';
  ...
  _.validateRegexPattern = function validateRegexPattern(pattern) {
  ...
  _.safeRegexMatch = function safeRegexMatch(pattern, str, flags = '') {
    const v = _.validateRegexPattern(pattern);
  ...
  _.filterCompatibleActions = function filterCompatibleActions(rule) {
    if (!rule?.actions || !rule.actions.forceType) return rule?.actions || null;
    const comp = _.TYPE_COMPATIBILITY_RULES[rule.actions.forceType];
  ...
  _.matchPreviewRule = function matchPreviewRule(url) {
    if (!_.settings.previewRules || _.settings.previewRules.length === 0 || !url) return null;
    ...
    try {
      ...
      const sortedRules = [..._.settings.previewRules]
        .filter(r => r.enabled && r.matchValue)
        .sort((a, b) => (b.priority || 0) - (a.priority || 0));

      for (const rule of sortedRules) {
        let matched = false;
        const rawMatchValue = rule.matchType === 'suffix' ? _.normalizeSuffix(rule.matchValue) : rule.matchValue;
        ...
        switch (rule.matchType) {
          ...
          case 'regex':
            ...
            matched = _.safeRegexMatch(rule.matchValue, fullUrl, flags);
            break;
          default:
            matched = false;
        }
        ...
        if (matched) {
          return { ...rule, actions: _.filterCompatibleActions(rule) };
        }
      }
    } catch (e) {
      return null;
